...
int dsERASE_MANAGER_LINE_ERASER_strid__GetThreadId();   /* dsERASE_MANAGER<LINE_ERASER<dsSTRID>>::GetThreadId @0x825284C8; disambiguated from the dsSTATE_MGR twin */

typedef dsLF_SORTED_MAP<const char *, dsSTRID, dsSTR_HASH, dsSTR_CMP>::LINE_ERASER StrStridLineEraser;

// dsERASE_MANAGER<dsLF_SORTED_MAP<char const*,dsSTRID,dsSTR_HASH,dsSTR_CMP>::LINE_ERASER>::CheckThread
// @ 0x82528730 — debug thread-affinity guard: assert the calling thread has a valid id and
// already owns an allocation bit in this manager before it traverses the lock-free container.
template<>
void dsERASE_MANAGER<StrStridLineEraser>::CheckThread()
{
    if (IGNORE_STRONG_ASSERT)
        return;

    int threadId = dsERASE_MANAGER_LINE_ERASER_strid__GetThreadId();
