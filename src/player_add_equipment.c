 ...
 * the inventory (deleting the object if it can't be carried); the profile's health/shield floats (entry 1)
 * and its two grenade-type counts (bytes +80/+81) are added to the unit.
 *
 * The starting profile is a 104-byte record viewed as an array of starting_weapon_info; entry 1's
 * weapon/rounds fields are reinterpreted as the starting health/shield floats. */
...
void player_add_equipment(int unit_index, int16_t profile_index, uint8_t reset)
{
    ...
    /* stride 104 = scenario_starting_profile; the decompiler reinterprets the record as a run of
     * 20-byte starting_weapon_info slots (profile[1..3] index into it), preserved verbatim below. */
    const starting_weapon_info *profile = (const starting_weapon_info *)
        &((scenario_starting_profile *)global_scenario->starting_profiles.address)[profile_index];
    ...
    if (profile[2].weapon.index != -1)
    {
        int weapon = create_weapon(&profile[2], unit_index);
    ...
    if (profile[3].weapon.index != -1)
    {
        int weapon = create_weapon(&profile[3], unit_index);
        if (weapon != -1 && !unit_add_weapon_to_inventory(unit_index, weapon, _unit_add_weapon_normally))
            object_delete(weapon);
    }

    unit->object.shield_vitality += *(float *)&profile[1].rounds_loaded;
    unit->object.body_vitality += *(float *)&profile[1].weapon.index;
    for (int i = 0; i < 2; i = (__int16)(i + 1))
        /* recovered: ((char *)profile)[i+80] -> profile[4].weapon.group_tag bytes (grenade-type counts) */
        unit->unit.grenade_counts[i] += ((const char *)&profile[4].weapon.group_tag)[i];
}
