/* message_object_deletion_definition @ 0x841767D0 (.data, 72 bytes) — the message-delta message definition for
 * `_message_object_deletion` (id 0): an object has been deleted on the server and must be removed on the client.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 1 body field.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x836EBD38 passes id 0.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000000 -> _message_object_deletion
 ...
 * 0x84176818, i.e. 72 bytes, and 0x28 + (1 + 1) * 16 == 72 — 16 being the DB member stride of
...
_message_definition message_object_deletion_definition =
{
    _message_object_deletion,                /* definition_type */
