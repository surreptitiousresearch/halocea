/* build_game_variant_reverse_tag @0x8374F100 — "Reverse Tag" variant: oddball engine (3) with ball type
 * "magic" (the ball marks its carrier as the one being hunted), no teams, 150-tick instant respawn, 2
 * points to win, weapon set 11, vehicle set 0x10000000, resets on capture.
 *
 * DEVIATION: the decompiler renders the flags OR-mask as `__ROL4__(65, 1) & 0x3F` — a rotate-and-mask of
 * two compile-time constants that always evaluates to the same value; folded here to its result (0x82). */
...
game_variant * build_game_variant_reverse_tag(game_variant *result)
{
    ...
    result->game_engine_variant.oddball.oddball_ball_type = _oddball_magic;
