...
struct fioFILE_MEM : fioFILE {
    ...
    virtual int Term();                          // 0x825B05F8 (overrides fioFILE::Term)
    ...
    virtual ~fioFILE_MEM();         // 0x825AE730 (overrides fioFILE::~fioFILE)

    // Extra vtable slots beyond fioFILE's 10 (types_members fioFILE_MEM_vtbl: GetBuf@40,
    // GetCurPtr@44) — bodies not decompiled here.
    virtual unsigned char *GetBuf();    // slot 10 boundary
    virtual unsigned char *GetCurPtr(); // slot 11 boundary
};
