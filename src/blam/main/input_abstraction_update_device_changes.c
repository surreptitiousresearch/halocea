...
#include <stdint.h>
#include "headers/input_abstraction_constants.h"
...
void input_abstraction_update_device_changes(unsigned int device_change_flags)
{
    if ( input_abstraction_globals.initialized )
    {
        if ( device_change_flags
            && (system_milliseconds() - input_abstraction_globals.device_enumeration_startup_timer
                    >= INPUT_DEVICE_ENUMERATION_TIME_MILLISECONDS
                || (time_of_first_device_insertion
                    && system_milliseconds() - time_of_first_device_insertion
                        >= INPUT_DEVICE_ENUMERATION_TIME_MILLISECONDS)) )
        {
            bink_playback_stop();
        }
