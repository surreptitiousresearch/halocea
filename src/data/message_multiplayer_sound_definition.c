/* message_multiplayer_sound_definition @ 0x84184B78 (.data, 72 bytes) — the message-delta message definition for
 * `_message_multiplayer_sound` (id 24): a multiplayer sound cue.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 1 body field.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x837B6914 passes id 24.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000018 -> _message_multiplayer_sound
 ...
 * 0x84184BC0, i.e. 72 bytes, and 0x28 + (1 + 1) * 16 == 72 — 16 being the DB member stride of
...
_message_definition message_multiplayer_sound_definition =
{
    _message_multiplayer_sound,              /* definition_type */
