/* unit_seat_is_gunner @0x836CADF8 — true if the given seat of a unit (vehicle) definition is a gunner
 * seat (unit_seat.flags bit 0x3). Resolves the object's unit/vehicle tag definition and indexes its
 * shared seats block. */
...
uint8_t unit_seat_is_gunner(int vehicle_index, int16_t seat_index)
{
    object_datum *object =
    ...
    if ( seat_index >= 0 && seat_index < definition->unit.seats.count )
    {
        ...
        return (seat->flags >> _unit_seat_is_gunner_bit) & 1;
    }
    return 0;
}
