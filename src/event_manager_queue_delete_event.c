/* event_manager_queue_delete_event @0x83726820 — queue a "delete" UI input event (button code 2) for a
...
void event_manager_queue_delete_event(uint8_t ticks, int16_t player_number)
{
    if (event_manager_globals.initialized)
    {
        event_record event;
        event.type = event_type_button;
        event.value.button_value.button = _gamepad_analog_button_x;
