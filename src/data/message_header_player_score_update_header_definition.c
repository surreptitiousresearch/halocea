/* message_header_player_score_update_header_definition @ 0x8417D9D4 (.data, 40 bytes) — the shared per-message HEADER
 * field-reference set "player_score_update_header": the fields every message carrying this header serializes ahead
 ...
 * DB applied_types: _message_definition_field_reference_set message_header_player_score_update_header_definition;
 * Image bytes (big-endian), decoded from the binary .data record:
 *   +0x000 count             = 0x00000001 -> 1
 ...
 *   +0x018   [ 1] = { 0x00000000, 0, 0, 0 }   <- terminator
 * Extent: field_references[] is the struct's trailing flexible array, so the object's extent
 * fixes its length. This symbol is 40 bytes and 0x8 + (1 + 1) * 16 == 40 — 16 being the DB member
 ...
 * zero offsets.
 * The object is 40 bytes; the 4 trailing zero byte(s) up to the next named symbol at
 * 0x8417DA00 are inter-symbol alignment padding, not part of it.
...
_message_definition_field_reference_set message_header_player_score_update_header_definition =
{
    1,                                       /* count */
    -1,                                      /* maximum_data_size */
