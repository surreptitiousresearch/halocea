/* message_header_game_object_header_definition @ 0x841845F8 (.data, 88 bytes) — the shared per-message HEADER
 * field-reference set "game_object_header": the fields every message carrying this header serializes ahead
 ...
 * DB applied_types: _message_definition_field_reference_set message_header_game_object_header_definition;
 * Image bytes (big-endian), decoded from the binary .data record:
 *   +0x000 count             = 0x00000004 -> 4
 *   +0x004 maximum_data_size = 0xFFFFFFFF -> -1
 *   +0x008   [ 0] = { 0x84183A30 -> &field_properties_object_index_definition, offset 0, baseline_offset 0, initialized 0 }
 *   +0x018   [ 1] = { 0x841832D0 -> &field_properties_integer_small_definition, offset 4, baseline_offset 4, initialized 0 }
 *   +0x028   [ 2] = { 0x841832D0 -> &field_properties_integer_small_definition, offset 5, baseline_offset 5, initialized 0 }
 *   +0x038   [ 3] = { 0x84183960 -> &field_properties_boolean_definition, offset 6, baseline_offset 6, initialized 0 }
 *   +0x048   [ 4] = { 0x00000000, 0, 0, 0 }   <- terminator
 * Extent: field_references[] is the struct's trailing flexible array, so the object's extent
 * fixes its length. This symbol is 88 bytes and 0x8 + (4 + 1) * 16 == 88 — 16 being the DB member
...
_message_definition_field_reference_set message_header_game_object_header_definition =
{
    4,                                       /* count */
    -1,                                      /* maximum_data_size */
    {   /* field_references[count + terminator]: { properties, offset, baseline_offset, initialized } */
        ...
        { &field_properties_integer_small_definition, 5, 5, 0 },  /* [ 2] */
