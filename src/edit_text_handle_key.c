...
void edit_text_handle_key(edit_text *edit, const key_stroke *key)
{
    ...
    int16_t index_a;  /* sp+50 scratch index */
    int16_t index_b;  /* sp+52 scratch index */
    ...
    if ( key_code == _key_backspace || key_code == _key_delete )
    {
        if ( !edit_text_get_selection_indices(edit, &index_a, &index_b) )  /* no selection */
        {
            ...
            {
                int16_t old_index = edit->insertion_point_index;
                get_previous_character((uint8_t *)edit->buffer, &edit->insertion_point_index);
            ...
            {
                uint8_t *buffer = (uint8_t *)edit->buffer;
                ...
                {
                    align_to_character((uint8_t *)edit->buffer, &edit->insertion_point_index);
                    return;
                }
        ...
        while ( *tail++ )
            ;
        int16_t selection_start = index_a;
    ...
    if ( key_code != _key_left_arrow && key_code != _key_right_arrow )  /* not an arrow key: character insertion */
    {
        ...
        {
            uint8_t has_selection = edit_text_get_selection_indices(edit, &index_b, &index_a);
            ...
            if ( has_selection )  /* replace selection with the typed character */
            {
                ...
                while ( *tail++ )
                    ;
                int16_t selection_start = index_b;
    ...
    if ( (key->modifier_flags & (1u << _key_modifier_shift_bit)) != 0 || !edit_text_get_selection_indices(edit, &index_b, &index_a) )
    {
        ...
        if ( (key->modifier_flags & (1u << _key_modifier_shift_bit)) != 0 && (uint16_t)edit->selection_start_index == 0xFFFF )
    ...
    int code = (uint16_t)key->key_code;
    edit->selection_start_index = -1;
