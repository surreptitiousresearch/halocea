...
{
    unsigned __int16 available_colors = player_profile_number_of_available_primary_colors();
    player_profile *edit_player_profile = player_ui_get_edit_player_profile();
    preview_list_create();

    if ( edit_player_profile )
    {
        __int16 stored_color_index = edit_player_profile->primary_color_index;
        __int16 clamped_color_index = available_colors - 1;
    ...
    if ( list_items )
    {
        ...
        for ( unsigned __int16 color_index = 0; color_index < available_colors; color_index++ )
        {
            ((unsigned __int8 *)list_items)[color_index] = (unsigned __int8)color_index;
