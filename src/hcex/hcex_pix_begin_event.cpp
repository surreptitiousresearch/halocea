/* hcex_pix_begin_event @0x823CED08 — format a message and open a named PIX profiler event, but only when PIX
 * instrumentation is enabled. vsnprintf into a 128-byte stack buffer (clamped to 127 chars + NUL) then
 * osPIXBeginEvent. Pairs with hcex_pix_end_event.
 *
 ...
 * (const char *fmt, ...). osPIXBeginEvent is a ws-engine profiling boundary. */
...
extern "C" void hcex_pix_begin_event(const char *fmt, ...)
{
    char buffer[128];
    va_list args;
