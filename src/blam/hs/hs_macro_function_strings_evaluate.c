...
uint8_t hs_macro_function_strings_evaluate(int thread_index, uint8_t initialize, int *count_out, const char ***strings_out)
{
    ...
    int *count = (int *)(((uintptr_t)stack_top + 3) & ~(uintptr_t)0x3); /* align up to 4 */
    ...
    char *values = (char *)(((uintptr_t)stack_top + 3) & ~(uintptr_t)0x3); /* align up to 4 */
    if ( (uintptr_t)(values - 4) > (uintptr_t)stack_top )   /* dead branch (shipped) */
        values -= 4;
    ...
    frame = thread->stack;
    stack_top = &frame->data[frame->size];
    int16_t *cursor = (int16_t *)(((uintptr_t)stack_top + 1) & ~(uintptr_t)0x1); /* align up to 2 */
    if ( (uintptr_t)(cursor - 1) > (uintptr_t)stack_top )   /* dead branch (shipped) */
        --cursor;
    ...
    frame = thread->stack;
    stack_top = &frame->data[frame->size];
    int *current_argument = (int *)(((uintptr_t)stack_top + 3) & ~(uintptr_t)0x3); /* align up to 4 */
