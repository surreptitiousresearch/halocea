/* ?GetAsBool@sslSTACK_DATA@@QBA?AVsslERROR@@AA_N@Z @0x82AF3468 */
...
// 0x82AF3468 — sslSTACK_DATA::GetAsBool. Returns sslERROR by value (sret): the decompiler swaps
// this/result (r3 = return slot sslERROR*, r4 = this sslSTACK_DATA*). Fetch this value's dsDATA
// form; on a fetch error, propagate it; otherwise math-convert to bool and read it out, or build a
// runtime "can't convert" error.
sslERROR sslSTACK_DATA::GetAsBool(bool &val) const
{
    dsDATA data;
    data.type = nullptr;
    ...
    if (data.Convert(dsDATA_TYPE_STORAGE<bool>::dataType, DSD_CONV_MATH))
    {
        data.GetValue<bool>(val, DSD_CONV_RETRIEVE);
        if (data.type)
            data.type->Destroy(&data.storage);
        return sslERROR();
    }
    ...
    dsTSTRING<char> msg;
    sslERROR err(SSL_ERR_RUNTIME, *dsSPrintf(&msg, "can't convert from <%s> to bool", typeName), 0);
    if (data.type)
        data.type->Destroy(&data.storage);
    return err;
}
