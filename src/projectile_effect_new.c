 ...
 * impulse_field = NULL) plus deterministic = 1 on the unattached path.
 *
 * DEVIATION: the DB prototype spells the 7th parameter `float __noop` — an IDA placeholder name that also
 * collides with the MSVC intrinsic — and an earlier note here read that as "the callee's scale_b receives
 * the caller's undefined f2". The disassembly refutes the second half. This function contains no
 * floating-point instruction at all (49 insns, none FP), so f1/f2 pass straight through to the callees'
 * scale_a/scale_b, but BOTH call sites materialize f2 explicitly: `fmr f2, f23` @0x8375BB14 and
 * @0x8375BB60, beside `fmr f1, f24` for `scale`. f23 is projectile_collision's material_effect_scale,
 * computed and clamped to [0,1] @0x8375BA14-0x8375BA2C — hence the name used here. */
...
void projectile_effect_new(int projectile_index, int definition_index, collision_result *collision,
        real_point3d *points, real_vector3d *vectors, float scale, float material_effect_scale)
{
