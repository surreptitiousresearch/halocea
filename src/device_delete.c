...
void device_delete(int device_index)
{
    ...
    __int16 group1 = device->device.power_group_index;
    if ( group1 != -1 && (groups[(unsigned __int16)group1].flags & (1u << _device_group_runtime_bit)) != 0 )
        datum_delete(device_groups_data, group1);

    __int16 group2 = device->device.position_group_index;
    if ( group2 != -1 && (groups[(unsigned __int16)group2].flags & (1u << _device_group_runtime_bit)) != 0 )
