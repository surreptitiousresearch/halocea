...
// dsVECTOR<sslFOREACH_DATA,8>::Realloc @ 0x82AF1880
// Resize the backing store to hold exactly `size` elements. ShrinkResize clamps the live count,
// then a single dlRealloc resizes the storage in place — sslFOREACH_DATA is trivially
// relocatable, so there is no per-element copy/move. Element size 8 bytes.
template<>
void dsVECTOR<sslFOREACH_DATA, 8>::Realloc(int size)
{
