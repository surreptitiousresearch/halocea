/* message_projectile_update_definition @ 0x8417EAF8 (.data, 88 bytes) — the message-delta message definition for
 * `_message_projectile_update` (id 1): a projectile's position and translational velocity.
 * Per-message header set: message_header_game_object_header_definition; 2 body fields.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000001 -> _message_projectile_update
 ...
 * 0x8417EB50, i.e. 88 bytes, and 0x28 + (2 + 1) * 16 == 88 — 16 being the DB member stride of
...
_message_definition message_projectile_update_definition =
{
    _message_projectile_update,              /* definition_type */
