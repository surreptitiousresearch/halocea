/* draw_string_compute_bounds @0x8376B288 — measure an ASCII/markup string laid out within `bounds`,
 * returning the tight text bounding rectangle (text_bounds) and the end-of-string cursor rectangle
 * (cursor_bounds). Sibling of draw_unicode_string_compute_bounds.c (same algorithm, draw_string instead
 * of draw_unicode_string): resolves the active font (applying the current style's font variant if any),
...
void draw_string_compute_bounds(const rectangle2d *bounds, const char *string, rectangle2d *text_bounds,
    rectangle2d *cursor_bounds)
{
    ...
    point2d pen;
    draw_string(text_bounds_draw_character, bounds, &pen, nullptr, 0, string);
