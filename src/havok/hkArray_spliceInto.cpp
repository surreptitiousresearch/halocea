...
/* hkArray<hkdShapeInstanceInfo>::spliceInto — replace numToRemove elements at
   `index` with numToInsert elements copied from insertData, growing the storage
   when the resulting size exceeds capacity.
   ...
   straight element copy. Element size is sizeof(hkdShapeInstanceInfo) == 80. */
void hkArray_spliceInto(hkArray<hkdShapeInstanceInfo> *self, int index, int numToRemove,
                        const hkdShapeInstanceInfo *insertData, int numToInsert)
{
    ...
    if (capacity < newSize)
    {
        ...
        if (newSize >= grow)
            grow = newSize;
        hkArrayUtil__reserve(&self->m_data, grow, (int)sizeof(hkdShapeInstanceInfo));
    }

    if (tailCount > 0)
        memmove(&self->m_data[index + numToInsert],
                &self->m_data[index + numToRemove],
                tailCount * (int)sizeof(hkdShapeInstanceInfo));
