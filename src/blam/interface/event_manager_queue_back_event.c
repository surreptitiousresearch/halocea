/* event_manager_queue_back_event @0x83726788 — queue a "back" UI input event (button code 13) for a
...
void event_manager_queue_back_event(uint8_t ticks, int16_t player_number)
{
    if (event_manager_globals.initialized)
    {
        event_record event;
        event.type = event_type_button;
        event.value.button_value.button = _gamepad_binary_button_back;
