/* message_player_handle_powerup_definition @ 0x84175CA8 (.data, 104 bytes) — the message-delta message definition for
 * `_message_player_handle_powerup` (id 14): a player has picked up or used a powerup.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 3 body fields.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x836AA430 passes id 14.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x0000000E -> _message_player_handle_powerup
 ...
 * 0x84175D10, i.e. 104 bytes, and 0x28 + (3 + 1) * 16 == 104 — 16 being the DB member stride of
...
_message_definition message_player_handle_powerup_definition =
{
    _message_player_handle_powerup,          /* definition_type */
