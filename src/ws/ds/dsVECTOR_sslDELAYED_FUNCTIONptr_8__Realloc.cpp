...
class sslDELAYED_FUNCTION; // boundary — ssl subsystem deferred-script record (element is a pointer)

// dsVECTOR<sslDELAYED_FUNCTION*,8>::Realloc @ 0x825CF730
...
template<>
void dsVECTOR<sslDELAYED_FUNCTION *, 8>::Realloc(int size)
{
    if (size != this->allocated) {
        ...
        if (!IGNORE_STRONG_ASSERT && size < this->nElem)
            STRONG_ASSERT2_HELPER::asserd<int, int>(
                ...
                "size", size,
                "nElem", this->nElem);
        this->pData = (sslDELAYED_FUNCTION **)dlRealloc(this->pData, 4 * size, this->__cl.file, this->__cl.line);
        this->allocated = size;
    }
}
