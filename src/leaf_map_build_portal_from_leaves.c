 ...
 * modeled leaf_map_portal {plane_index, leaf_index0, leaf_index1, vertices}. Leaves keep the decompiler's raw
 * tag_block[2] view (faces block + designator block); faces are map_leaf_face {node_index, vertices tag_block}. The
 * leaf stride idiom 16*n + 8*(n & 0x7FFFFFFF) is 24*n for a non-negative n, reproduced verbatim. */
...
{
    char *leaves = (char *)leaf_map->leaves.address;
    ...
    map_leaf_face *face0_ptr = &((map_leaf_face *)leaf0->address)[face0];
    map_leaf_face *face1_ptr = &((map_leaf_face *)leaf1->address)[face1];
    ...
    int designator0 = tag_block_add_element(leaf0 + 1);
    int designator1 = tag_block_add_element(leaf1 + 1);
    ...
    *((int *)leaf1[1].address + designator1) = portal_index;
