/* message_remote_player_position_update_definition @ 0x8417E128 (.data, 72 bytes) — the message-delta message definition for
 * `_message_remote_player_position_update` (id 38): a remote player's position update.
 * Per-message header set: message_header_remote_player_position_update_header_definition; 1 body field.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x837572C8 passes id 38.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000026 -> _message_remote_player_position_update
 ...
 *   +0x20 body_fields.count                  = 0x00000001 -> 1
 *   +0x24 body_fields.maximum_data_size      = 0xFFFFFFFF -> -1
 *   +0x28   field_references[ 0]             = { 0x841837C0 -> &field_properties_locality_reference_position_definition, offset 0, baseline_offset 0, initialized 0 }
 *   +0x38   field_references[ 1]             = { 0x00000000, 0, 0, 0 }  <- terminator
 *
 ...
 * 0x8417E170, i.e. 72 bytes, and 0x28 + (1 + 1) * 16 == 72 — 16 being the DB member stride of
...
_message_definition message_remote_player_position_update_definition =
{
    _message_remote_player_position_update,  /* definition_type */
    ...
    {                                        /* body_fields */
        ...
        {   /* field_references[count + terminator]: { properties, offset, baseline_offset, initialized } */
            { &field_properties_locality_reference_position_definition, 0, 0, 0 },  /* [ 0] */
            { 0, 0, 0, 0 },                                                         /* [ 1] terminator */
        },
    },
};
