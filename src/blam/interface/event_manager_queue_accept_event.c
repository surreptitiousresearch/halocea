/* event_manager_queue_accept_event @0x83726738 — queue a "accept" UI input event (button code 0) for a
...
void event_manager_queue_accept_event(uint8_t ticks, int16_t player_number)
{
    if (event_manager_globals.initialized)
    {
        event_record event;
        event.type = event_type_button;
        event.value.button_value.button = _gamepad_analog_button_a;
