 ...
 * DEVIATION: declared void — the Release() HRESULT left in r3 at blr (0x83785F2C) is loop residue and
...
void rasterizer_occulsion_test_dispose(void)
{
    for (int i = 0; i < 1024; ++i)
    {
        if (occlusion_query[i])
        {
            D3DQuery_Release(occlusion_query[i]);
