/* message_header_client_game_update_header_definition @ 0x8417DD38 (.data, 40 bytes) — the shared per-message HEADER
 * field-reference set "client_game_update_header": the fields every message carrying this header serializes ahead
 ...
 * DB applied_types: _message_definition_field_reference_set message_header_client_game_update_header_definition;
 * Image bytes (big-endian), decoded from the binary .data record:
 *   +0x000 count             = 0x00000001 -> 1
 *   +0x004 maximum_data_size = 0xFFFFFFFF -> -1
 *   +0x008   [ 0] = { 0x84183268 -> &field_properties_update_id_definition, offset 0, baseline_offset 0, initialized 0 }
...
_message_definition_field_reference_set message_header_client_game_update_header_definition =
{
    1,                                       /* count */
    -1,                                      /* maximum_data_size */
    {   /* field_references[count + terminator]: { properties, offset, baseline_offset, initialized } */
        { &field_properties_update_id_definition, 0, 0, 0 },  /* [ 0] */
