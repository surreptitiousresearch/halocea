 ...
 *    real calls are `pow(values[power_high-1], shader[1].base.radiosity.color.n[0])` and
 *    `pow(group->animation->values[green_high-1], shader[1].base.radiosity.tint_color.n[0])`, f1/f2 loaded
 ...
 *  - The glow-color constant block reuses ONE raw (unmultiplied) float, `shader[7].base.radiosity.flags`
 *    (loaded once at 0x8382DE78 as f13), at three separate output slots (c13.w/c15.z/c18.x in register
 *    terms; array indices 12/17/22 here) — the decompiler's `v48[12]`/`v48[17]`/`v48[22]` all showed the
 *    unrelated `shader[5].base.radiosity.flags` value (this file's `scaled_flags`) instead, because both
...
void rasterizer_plasma_energy_draw(const transparent_geometry_group *group)
{
    ...
    if ( animation )
    {
        if ( animation->colors )
        {
            __int16 color_index = *(__int16 *)&shader[3].base.radiosity.color.n[0];
        ...
        if ( values )
        {
            __int16 power_high = *(__int16 *)&shader[1].base.radiosity.power;
            if ( power_high >= 1 && power_high <= 4 )
                glow_scale = (float)pow(values[power_high - 1], shader[1].base.radiosity.color.n[0]);

            __int16 green_high = *(__int16 *)&shader[1].base.radiosity.color.__s1.green;
            if ( green_high >= 1 && green_high <= 4 )
            {
                tint_blue_scale = (float)pow(group->animation->values[green_high - 1],
                        shader[1].base.radiosity.tint_color.n[0]) * shader[1].base.radiosity.color.n[2];
    ...
    float scaled_type = *(float *)&shader[4].base.type;
    float scaled_flags = *(float *)&shader[5].base.radiosity.flags;
    float shader7_flags = *(float *)&shader[7].base.radiosity.flags;   /* raw, unscaled — reused 3x below */

    float time_over_period_1 = (float)(global_frame_parameters.game_time_sec
            / shader[6].base.radiosity.tint_color.n[1]);
    float time_over_period_2 = (float)(global_frame_parameters.game_time_sec
            / *(float *)&shader[4].base.physics);

    /* c13..c18: four repeated RGB triples (glow color / 0 / tint-blue) plus per-triple alpha-time terms */
    float glow_color_constants[24];
    glow_color_constants[0]  = shader[5].base.radiosity.color.n[0];
    glow_color_constants[1]  = 0.0f;
    glow_color_constants[2]  = tint_blue_scale;
    glow_color_constants[3]  = scaled_type * time_over_period_2;
    glow_color_constants[4]  = 0.0f;
    glow_color_constants[5]  = shader[5].base.radiosity.color.n[0];
    glow_color_constants[6]  = 0.0f;
    glow_color_constants[7]  = scaled_flags * time_over_period_2;
    glow_color_constants[8]  = 0.0f;
    glow_color_constants[9]  = 0.0f;
    glow_color_constants[10] = shader[5].base.radiosity.color.n[0];
    glow_color_constants[11] = shader[5].base.radiosity.power * time_over_period_2;
    glow_color_constants[12] = shader7_flags;
    glow_color_constants[13] = 0.0f;
    glow_color_constants[14] = 0.0f;
    glow_color_constants[15] = (shader[6].base.radiosity.tint_color.n[2] * time_over_period_1);
    glow_color_constants[16] = 0.0f;
    glow_color_constants[17] = shader7_flags;
    glow_color_constants[18] = 0.0f;
    glow_color_constants[19] = (*(float *)&shader[6].base.physics * time_over_period_1);
    glow_color_constants[20] = 0.0f;
    glow_color_constants[21] = 0.0f;
    glow_color_constants[22] = shader7_flags;
    glow_color_constants[23] = *(float *)&shader[6].base.type * time_over_period_1;
    D3DDevice_SetVertexShaderConstantFN(global_d3d_device, 0xD, glow_color_constants, 6,
            (unsigned __int64)3 << 59);

    rasterizer_set_texture_for_effect(0, 1, 0, *(int *)&shader[5].base.radiosity.tint_color.n[1],
            group->shader_permutation_index, dxeffect_shader);
    D3DDevice_SetSamplerState_AddressW_Inline(global_d3d_device, 0, 0);
    rasterizer_set_texture_for_effect(1, 1, 0, *(int *)&shader[7].base.radiosity.color.n[2],
            group->shader_permutation_index, dxeffect_shader);
    ...
    texture_transform_constants[4]  = (shader[2].base.radiosity.tint_color.n[0] - *(float *)&shader[2].base.type)
            * animation_color->n[0];
    texture_transform_constants[5]  = (shader[2].base.radiosity.tint_color.n[1]
            - *(float *)&shader[3].base.radiosity.flags) * animation_color->n[1];
    texture_transform_constants[6]  = (shader[2].base.radiosity.tint_color.n[2] - shader[3].base.radiosity.power)
            * animation_color->n[2];
    texture_transform_constants[7]  = (shader[2].base.radiosity.color.n[2] - *(float *)&shader[2].base.physics)
            * glow_scale;
    texture_transform_constants[8]  = *(float *)&shader[2].base.type * animation_color->n[0];
    texture_transform_constants[9]  = *(float *)&shader[3].base.radiosity.flags * animation_color->n[1];
    texture_transform_constants[10] = shader[3].base.radiosity.power * animation_color->n[2];
    texture_transform_constants[11] = *(float *)&shader[2].base.physics * glow_scale;
