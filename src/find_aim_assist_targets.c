...
extern __int16 find_aim_assist_targets_recursive(const aim_assist_parameters *parameters, int object_index,
...
int16_t find_aim_assist_targets(const aim_assist_parameters *parameters, int16_t position_cluster_index, const real_point3d *position, const real_vector3d *direction, aim_assist_target *ignore_object_index, int16_t ignore_team_index, int16_t maximum_target_count, aim_assist_target *targets)
{
    ...
    __int16 cluster_count = structure_clusters_in_cone(position_cluster_index, position, direction,
                                                       cone_distance, angle_sine, angle_cosine, 512,
                                                       cluster_indices);
    __int16 object_count = objects_in_clusters_by_indices(1u, cluster_count, cluster_indices, 2048,
                                                          object_indices);
    if (object_count <= 0)
        return 0;

    int total_found = 0;
    for (__int16 i = 0; i < object_count; i++)
    {
        int found = find_aim_assist_targets_recursive(parameters, object_indices[i], position, direction,
                                                       ...
                                                       &targets[total_found]);
        total_found = (__int16)(found + total_found);
