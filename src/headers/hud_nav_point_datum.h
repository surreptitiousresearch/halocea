...
{
    int16_t          nav_index;         /* 0x0 */
    uint16_t _reserved : 8;     /* 0x2 — reserved padding, bits [7:0] (DB anonymous bitfield) */
    int16_t          screen_type : 4;   /* 0x2 — bits [11:8], on-screen render category */
    int16_t          type : 4;          /* 0x2 — bits [15:12], nav-point type (0xF/-1 == deactivated) */
    float            z_offset;          /* 0x4 */
