/* message_team_change_definition @ 0x841853B0 (.data, 88 bytes) — the message-delta message definition for
 * `_message_team_change` (id 25): a player has changed team.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 2 body fields.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x838029D8 passes id 25.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000019 -> _message_team_change
 ...
 *   +0x38   field_references[ 1]             = { 0x841832D0 -> &field_properties_integer_small_definition, offset 1, baseline_offset 1, initialized 0 }
 ...
 * 0x84185408, i.e. 88 bytes, and 0x28 + (2 + 1) * 16 == 88 — 16 being the DB member stride of
...
_message_definition message_team_change_definition =
{
    _message_team_change,                    /* definition_type */
