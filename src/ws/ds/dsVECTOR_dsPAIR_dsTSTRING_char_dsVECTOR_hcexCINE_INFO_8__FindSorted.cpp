...
// dsVECTOR<dsPAIR<dsTSTRING<char>,dsVECTOR<hcexCINE_INFO,8>>,8>::FindSorted<dsCMP,dsTSTRING<char>> @ 0x823B5218
// Binary search of listCine (script-name -> per-difficulty actor/mask table), kept ascending by
// string key, for the pair whose key equals `key`; returns its index or -1. The comparator is
// inlined as a byte-wise string compare, applied twice per probe (mid-vs-key to pick a half, then
// key-vs-mid to confirm equality).
template<>
template<>
int dsVECTOR<dsPAIR<dsTSTRING<char>, dsVECTOR<hcexCINE_INFO, 8> >, 8>::FindSorted<dsCMP, dsTSTRING<char> >(
        const dsTSTRING<char> &key, const dsCMP &cmp) const
{
    ...
    if (hi) {
        ...
        const dsPAIR<dsTSTRING<char>, dsVECTOR<hcexCINE_INFO, 8> > *elems = this->pData;
        do {
            int mid = (hi + lo) >> 1;
