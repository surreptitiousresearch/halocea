/* event_manager_queue_left_event @0x83726698 — queue a "left" UI input event (button code 10) for a
...
void event_manager_queue_left_event(uint8_t ticks, int16_t player_number)
{
    if (event_manager_globals.initialized)
    {
        event_record event;
        event.type = event_type_button;
        event.value.button_value.button = _gamepad_binary_button_dpad_left;
