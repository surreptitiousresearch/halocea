 ...
 *
 * DEVIATION: the 21 declaring TUs do not agree. 16 spell it `const int16_t[1][6][2]` and index
 * `[0][2*axis+sign][c]`, 4 spell `[3][2][2]`, 1 spells `[][2]`. All three spellings compute the
 * same byte offsets for the subscripts they actually use, so none of them miscompiles today, but
 * only [3][2][2] is the real shape. Consumers are not edited here (declaration normalization is
 * owned elsewhere) — see the slice report. */
