...
extern void *bitmap_3d_address(const bitmap_data *bitmap, __int16 x, __int16 y, __int16 z, __int16 mipmap_index);
...
bitmap_data * bitmap_3d_shrink(const bitmap_data *source_bitmap, int16_t scale, int16_t alpha_bias, uint8_t ignore_zero_alpha)
{
    ...
    __int16 sample_height = scale <= source_height ? scale : source_height;
    __int16 source_depth = source_bitmap->depth;
    __int16 sample_depth = scale > source_depth ? source_depth : scale;
    ...
    {
        for ( __int16 z_out = 0; z_out < out_depth; ++z_out )
        {
            for ( __int16 y_out = 0; y_out < out_height; ++y_out )
            {
                for ( __int16 x_out = 0; x_out < out_width; ++x_out )
                {
                    ...
                    for ( __int16 sz = 0; sz < sample_depth; ++sz )
                    {
