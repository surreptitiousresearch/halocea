/* build_game_variant_accumulation @0x8374F1E8 — "Accumulation" variant: oddball engine (3) with ball type
 * "magic", no teams, 150-tick instant respawn, 5 points to win, weapon set 11, vehicle set 0x10000000,
 * dual goal radar mode (2), 16 balls spawned.
 *
 * DEVIATION: as in build_game_variant_reverse_tag, the decompiler's flags OR-mask
 * `__ROL4__(65, 1) & 0x3F` is a compile-time-constant rotate/mask; folded to its result (0x82). */
...
game_variant * build_game_variant_accumulation(game_variant *result)
{
