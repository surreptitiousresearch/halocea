...
typedef struct weapon_new_data
{
    ...
    uint8_t current_baseline_index;  /* 0x48 — weapon.baseline_index (object +737) */
    unsigned char  _pad49[1];                /* 0x49 */
    int16_t        magazine_0_rounds_total;  /* 0x4A — weapon.baseline (object +776) */
    int16_t        magazine_1_rounds_total;  /* 0x4C — weapon.baseline (object +778) */
    unsigned char  _pad4E[2];                /* 0x4E */
    float          age;                      /* 0x50 — weapon.baseline.age (object +780) */
    int16_t        magazine_0_rounds_loaded; /* 0x54 — weapon.magazines[0].rounds_loaded (object +696) */
    int16_t        magazine_1_rounds_loaded; /* 0x56 — weapon.magazines[1].rounds_loaded (object +708) */
