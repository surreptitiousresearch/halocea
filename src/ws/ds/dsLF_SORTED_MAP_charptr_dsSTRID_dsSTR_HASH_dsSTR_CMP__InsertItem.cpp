...
// dsLF_SORTED_MAP<const char*,dsSTRID,dsSTR_HASH,dsSTR_CMP>::InsertItem @ 0x8252B460
...
// return the new node.
template<>
dsLF_SORTED_MAP<const char *, dsSTRID, dsSTR_HASH, dsSTR_CMP>::LINE *
dsLF_SORTED_MAP<const char *, dsSTRID, dsSTR_HASH, dsSTR_CMP>::InsertItem(
        LINE *line, int index, const ITEM *item)
{
    ...
    if (&newLine->items[index] != 0) {
        ...
        newLine->items[index].val.id = item->val.id;
    }
    return newLine;
}
