...
// colored green for a streamed sound-list and amber for a fully memory-resident one (disasm-
...
void HALO_SOUND_SYSTEM::DbgRenderSounds()
{
    ...
    for (int i = 0; i < channels.nElem; ++i) {
        ...
        if (chan.current.fsbIndex != -1) {
            ...
            unsigned long color = chan.current.soundList->isMemory ? 0x903F10ul : 0x10FF10ul;
