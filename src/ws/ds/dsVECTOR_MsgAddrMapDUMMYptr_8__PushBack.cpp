...
// dsVECTOR<...msgADDR-map DUMMY*,8>::PushBack @ 0x8255A698
// Append `val` to the tail, growing the backing store first when there is no spare capacity, and
// return a reference to the newly stored pointer slot.
template<>
dsMsgAddrMapDUMMY *&dsVECTOR<dsMsgAddrMapDUMMY *, 8>::PushBack(dsMsgAddrMapDUMMY *const &val)
{
