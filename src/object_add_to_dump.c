...
void object_add_to_dump(int object_index, dump_datum *dump)
{
    ...
    __int16 datum_size = datum->data_size;
    ...
    unsigned __int16 ultimate_parent = object_get_ultimate_parent(object_index);
    ...
    if ((parent_object->object.flags & (1u << _object_outside_of_map_bit)) != 0
        || (unsigned __int16)parent_object->object.location.cluster_index == 0xFFFF)
