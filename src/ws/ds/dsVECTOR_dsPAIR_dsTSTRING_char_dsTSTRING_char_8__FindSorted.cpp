...
// dsVECTOR<dsPAIR<dsTSTRING<char>,dsTSTRING<char>>,8>::FindSorted<dsCMP,dsTSTRING<char>> @ 0x823D6160
// Binary search of a vector kept ascending by string key for the pair whose key equals `key`;
// returns its index or -1. The comparator is inlined as a byte-wise string compare, applied
// twice per probe (mid-vs-key to pick a half, then key-vs-mid to confirm equality).
template<>
template<>
int dsVECTOR<dsPAIR<dsTSTRING<char>, dsTSTRING<char> >, 8>::FindSorted<dsCMP, dsTSTRING<char> >(
        const dsTSTRING<char> &key, const dsCMP &cmp) const
{
    ...
    if (hi) {
        ...
        const dsPAIR<dsTSTRING<char>, dsTSTRING<char> > *elems = this->pData;
        do {
            int mid = (hi + lo) >> 1;
