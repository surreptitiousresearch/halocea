...
// aiPLANNER::NoticeFlyingRocket(iaIACTOR*) @ 0x83254518
// ?NoticeFlyingRocket@aiPLANNER@@QAAXPAViaIACTOR@@@Z
//
// Run the descriptor's "NoticeFlyingRocket" SSL function with `actor`'s SSL object reference (an
// empty ref when null) as its single argument (no return value used).
void aiPLANNER::NoticeFlyingRocket(iaIACTOR *actor)
{
    ...
    aiPLANNER_IA(this).sslObject.CallFunc(
        aiPLANNER_Desc(this)->sslFuncId_NoticeFlyingRocket, 1, &arg, nullptr, sslOBJ_REF());
