/* _rasterizer_environment_reflection_draw @0x837A7F38 — draw the cube-map reflection pass for one environment
 * surface batch. Runs when the reflection debug toggle is on, the shader reflects (positive perpendicular or
 * parallel brightness), and a reflection cube map is present. Selects a reflection effect shader by the
 * reflection mode and the colored/flat-lightmap flag, binds the bump map (stage 0), the normalization cube
 * map (stages 1/2), and the reflection cube map (stage 3), uploads the bump texture-transform constants (with
 * a 320x240 projection scale), the camera-forward vector, and the reflection tint/brightness constants, then
 * draws the dynamic triangles once per effect pass. Under real-time lightmaps in reflection mode 2 the second
 * static-vertex stream is taken from the next vertex buffer.
 *
 ...
 * DEVIATION 3: the texture-transform constants the decompiler split into v46[7]+v47[5] are one contiguous
 * 12-float block uploaded as 3 vector4s; evaluate writes the u/v scroll offsets into elements [7] and [11].
 * The decompiler reuses one 4-float scratch for all three SetVector uploads; expressed here as three named
 * buffers (each is copied by SetVector before the next fill, so behavior is identical).
 * FAITHFUL QUIRK: the reflection-mode == 3 branch selects the effect index from the high half-word of the
 * still-uninitialized effect pass-count buffer (a latent bug; reproduced verbatim). */
...
void _rasterizer_environment_reflection_draw(const shader *shader, int16_t shader_permutation_index, int dynamic_triangle_buffer_index, int first_triangle_index, int triangle_count, const vertex_buffer *vertex_buffer)
{
    ...
    if (!(shader_env->environment.reflection.view_perpendicular_brightness > 0.0f
        ...
        || shader_env->environment.reflection.map.index == -1)
        return;
    ...
    rasterizer_set_texture_direct_for_effect(2, global_rasterizer_data->vector_normalization.index, 0,
                                             effect_shader);
    rasterizer_set_texture_for_effect(3, 2, 0, shader_env->environment.reflection.map.index,
                                      shader_permutation_index, effect_shader);

    /* c10..c12 texture-transform block; [2]/[3] carry the 320x240 projection scale. */
    ...
    if (constants)
    {
        /* c0: camera forward remapped to [-1,1] (xyz), w = 0. */
        float camera_constants[4];
        ...
        camera_constants[0] = forward_x * 2.0f - 1.0f;
        ...
        camera_constants[1] = forward_y * 2.0f - 1.0f;
        ...
        camera_constants[2] = forward_z * 2.0f - 1.0f;
        camera_constants[3] = 0.0f;
        effect_shader->effect->lpVtbl->SetVector(effect_shader->effect, *constants,
                                         (const D3DXVECTOR4 *)camera_constants);

        /* c1: reflection perpendicular tint + perpendicular brightness in w. */
        ...
        /* c2: reflection parallel tint + parallel brightness in w. */
    ...
    for (unsigned int i = 0; i < pass_count[0]; ++i)
    {
        ...
        int second_stream_offset =
            (rasterizer_globals.lightmap_mode && reflection_mode == _shader_environment_reflection_type_radiosity) ? 1 : 0;
        rasterizer_draw_dynamic_triangles_static_vertices2(dynamic_triangle_buffer_index, first_triangle_index,
                                                           ...
                                                           &vertex_buffer[second_stream_offset]);
