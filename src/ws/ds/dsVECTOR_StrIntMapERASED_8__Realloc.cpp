...
// dsVECTOR<dsERASE_MANAGER<...int-map LINE_ERASER>::ERASED,8>::Realloc @ 0x82617470
...
template<>
void dsVECTOR<dsStrIntMapERASED, 8>::Realloc(int size)
{
    if (size != this->allocated) {
        ...
        if (!IGNORE_STRONG_ASSERT && size < this->nElem)
            STRONG_ASSERT2_HELPER::asserd<int, int>(
                ...
                "size", size,
                "nElem", this->nElem);
        this->pData = (dsStrIntMapERASED *)dlRealloc(this->pData, 12 * size, this->__cl.file, this->__cl.line);
        this->allocated = size;
    }
}
