/* message_multiplayer_hud_message_definition @ 0x8417D948 (.data, 104 bytes) — the message-delta message definition for
 * `_message_multiplayer_hud_message` (id 23): a multiplayer HUD message directed at one player.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 3 body fields.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x8374D730 passes id 23.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000017 -> _message_multiplayer_hud_message
 ...
 * 0x8417D9B0, i.e. 104 bytes, and 0x28 + (3 + 1) * 16 == 104 — 16 being the DB member stride of
...
_message_definition message_multiplayer_hud_message_definition =
{
    _message_multiplayer_hud_message,        /* definition_type */
