/* message_parameters_protocol_definition @ 0x841856D8 (.data, 88 bytes) — the message-delta message definition for
 * `_message_parameters_protocol` (id 33): the message-delta parameters protocol handshake.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 2 body fields.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x83812248 passes id 33.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000021 -> _message_parameters_protocol
 ...
 *   +0x38   field_references[ 1]             = { 0x84185670 -> &field_properties_parameters_protocol_array_definition, offset 4, baseline_offset 4, initialized 0 }
 ...
 * 0x84185730, i.e. 88 bytes, and 0x28 + (2 + 1) * 16 == 88 — 16 being the DB member stride of
...
extern _field_properties_definition field_properties_parameters_protocol_array_definition;                         /* 0x84185670 */

_message_definition message_parameters_protocol_definition =
{
    _message_parameters_protocol,            /* definition_type */
    ...
    {                                        /* body_fields */
        ...
        {   /* field_references[count + terminator]: { properties, offset, baseline_offset, initialized } */
            ...
            { &field_properties_parameters_protocol_array_definition, 4, 4, 0 },  /* [ 1] */
