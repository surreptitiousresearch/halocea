/* ?GetAsFloat@sslSTACK_DATA@@QBA?AVsslERROR@@AAM@Z @0x82AF3760 */
...
// 0x82AF3760 — sslSTACK_DATA::GetAsFloat. Returns sslERROR by value (sret): the decompiler swaps
// this/result (r3 = return slot sslERROR*, r4 = this sslSTACK_DATA*). Fetch this value's dsDATA
// form; on a fetch error, propagate it; otherwise math-convert to float and read it out, or build a
// runtime "can't convert" error.
sslERROR sslSTACK_DATA::GetAsFloat(float &val) const
{
    dsDATA data;
    data.type = nullptr;
    ...
    if (data.Convert(dsDATA_TYPE_STORAGE<float>::dataType, DSD_CONV_MATH))
    {
        data.GetValue<float>(val, DSD_CONV_RETRIEVE);
        if (data.type)
            data.type->Destroy(&data.storage);
        return sslERROR();
    }
    ...
    dsTSTRING<char> msg;
    sslERROR err(SSL_ERR_RUNTIME, *dsSPrintf(&msg, "can't convert from <%s> to float", typeName), 0);
    if (data.type)
        data.type->Destroy(&data.storage);
    return err;
}
