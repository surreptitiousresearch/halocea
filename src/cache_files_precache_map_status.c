 ...
 *
 * Deviation: for an out-of-range status (>4) the decompiler returns an uninitialized stack slot (degenerate
 * path that the producer never reaches); reproduced verbatim. The redundant `&& status` guards the decompiler
 * emitted are dropped as no-ops. */
...
{
    uint16_t uninitialized;   /* decompiler v3: returned on the unreachable out-of-range path */
