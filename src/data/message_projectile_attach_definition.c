/* message_projectile_attach_definition @ 0x8417E9A8 (.data, 104 bytes) — the message-delta message definition for
 * `_message_projectile_attach` (id 46): a projectile has attached to an object.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 3 body fields.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x83759888 passes id 46.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x0000002E -> _message_projectile_attach
 ...
 * 0x8417EA10, i.e. 104 bytes, and 0x28 + (3 + 1) * 16 == 104 — 16 being the DB member stride of
...
_message_definition message_projectile_attach_definition =
{
    _message_projectile_attach,              /* definition_type */
