...
// dsVECTOR<short,8>::PushBack @ 0x823DFBA8
...
template<>
short &dsVECTOR<short, 8>::PushBack(const short &val)
{
    if (this->allocated >= this->nElem + 1) {
        this->pData[this->nElem] = val;
    } else {
        short saved = val; // save first: `val` may alias storage about to be reallocated
