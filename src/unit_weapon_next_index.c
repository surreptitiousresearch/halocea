...
{
    int16_t start_slot = current_index == -1 ? 0 : current_index;
    int best_slot = -1;
    ...
    do
    {
        ...
        int weapon_index = unit_object->unit.weapon_object_indices[(int16_t)slot];
        if ( weapon_index != -1 && unit_can_use_weapon(unit_index, weapon_index) )
        {
            if ( delta
              || (int16_t)best_slot == -1
              || unit_object->unit.weapon_last_used_at_game_time[(int16_t)best_slot]
                   < unit_object->unit.weapon_last_used_at_game_time[(int16_t)slot] )
            {
                best_slot = slot;
            }
            if ( weapon_must_be_readied(unit_object->unit.weapon_object_indices[(int16_t)slot])
              || (int16_t)slot != start_slot )
                break;
        }
        ...
        {
            next_slot = 0;
            if ( (int16_t)slot != MAXIMUM_WEAPONS_PER_UNIT - 1 )
                next_slot = slot + 1;
        }
        else
        {
