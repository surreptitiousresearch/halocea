 ...
 *
 * DEVIATION: same as the reflections begin — the inlined sampler setters were emitted as raw GPU fetch-constant
 * pokes (decompiler "local variable allocation has failed"); restored to D3DDevice_SetSamplerState_*_Inline.
 * No texture binds. The garbled MagFilter values are 1 (point) per the disassembly. */
...
void _rasterizer_environment_reflection_mirrors_begin(void)
{
    ...
    D3DDevice_SetSamplerState_AddressW_Inline(global_d3d_device, 1, 2);
