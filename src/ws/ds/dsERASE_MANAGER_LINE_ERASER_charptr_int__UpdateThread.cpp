...
// manager type the assigned allocation ordinal lives at offset 32. // boundary
struct dsLF_THREAD_INFO {
    char _pad[32]; // 0x00
    int  slotIndex; // 0x20  assigned allocation ordinal, or -1 when this thread is unregistered
};
dsLF_THREAD_INFO *dsLF_GetThreadInfo(); // boundary — reads the record pointer from TLS (r13)

// dsERASE_MANAGER<dsLF_SORTED_MAP<char const*,int,dsSTR_HASH,dsSTR_CMP>::LINE_ERASER>
//   ::UpdateThread @ 0x8261A128
...
// (Int-valued-map twin of the dsSTRID-valued UpdateThread; differs only in TLS slot offset.)
template<>
void dsERASE_MANAGER<StrIntLineEraser>::UpdateThread()
{
