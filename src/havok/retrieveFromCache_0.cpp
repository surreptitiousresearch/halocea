...
/* retrieveFromCache_0 @0x838C5D88
   Delta-compressed sibling of retrieveFromCache (the wavelet variant).
   Returns the decompressed DOF block for pose `poseIdx` of a delta-compressed
   ...
   allocated, the block is decompressed into it via decompressBlockToCache_0, and
   ...
   command pair (HK_TIMER_BEGIN("cacheD") / HK_TIMER_END). */
...
/* Sibling delta block decompressor @0x838C5C60 — already reversed. */
extern void decompressBlockToCache_0(
        const hkaDeltaCompressedAnimation *animation,
        unsigned int pose,
        float *outputCache,
        unsigned int numD);

float *retrieveFromCache_0(
        hkaDeltaCompressedAnimation *animation,
        ...
        hkaChunkCache **cache)
{
    ...
    /* HK_TIMER_BEGIN("cacheD") */
    {
        ...
        {
            beginCommand->m_commandAndMonitor = "TtcacheD";
...

    {
        ...
        hkaDeltaCompressedAnimation_vtbl *animVtbl =
            *(hkaDeltaCompressedAnimation_vtbl **)animation;
        ...
        else
        {
            ...
            decompressBlockToCache_0(animation, poseIdx, chunk, numD);
