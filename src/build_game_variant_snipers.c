/* build_game_variant_snipers @0x8374EDB0 — default "Snipers" variant: Slayer (engine 2) to 15 kills with
 * the sniper weapon set (4), a 150-tick respawn growth, and universal flag bits 0xA2 (the compiled
 * __ROL4__(81,1)&0x3F form, as in Rockets). */
...
game_variant * build_game_variant_snipers(game_variant *result)
{
    ...
    result->flags = 1;
    result->universal_variant.flags = (flags & (~((1u << NUMBER_OF_GAME_ENGINE_UNIVERSAL_VARIANT_FLAGS) - 1u) | (1u << _motion_tracker_no_enemies_bit))) | (1u << _allow_friendly_navpoints_bit) | (1u << _generic_starting_equipment_bit) | (1u << _game_mode_classic_bit);   /* __ROL4__(81,1)&0x3F | 0x80 */
    return result;
}
