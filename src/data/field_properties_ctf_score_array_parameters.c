/* field_properties_ctf_score_array_parameters @ 0x841854D4 (.data, 12 bytes) — the `parameters` blob for an
 * "array" message-delta field: 2 elements of 4 source byte(s) each, every element encoded and
 * decoded through the one shared member field-properties definition below.
 * DB applied_types: _field_type_array_parameters field_properties_ctf_score_array_parameters;
 * Image bytes (big-endian), decoded from the binary .data record:
 *   +0x00 count               = 0x00000002 -> 2
...
_field_type_array_parameters field_properties_ctf_score_array_parameters =
{
    2,                                      /* count */
