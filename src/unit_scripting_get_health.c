/* unit_scripting_get_health @0x83800970 — return a unit's health fraction (object data +0xE0), 0 if it is
...
float unit_scripting_get_health(int unit_index)
{
    ...
    if ( (unit->object.damage_flags & (1u << _object_dead_bit)) != 0 )
        return 0.0f;
    return unit->object.body_vitality;
}
