...
struct hcexHALO_CUSTOM_OBJ_LIST : instMANAGER_HANDLER {
    ...
    // 0x823D9518 — register the vftable, default-construct cdList (adopting the
    // "halo_main.cpp":222 allocation call-site cookie).
    hcexHALO_CUSTOM_OBJ_LIST();

    // 0x823D97D8 — tear down cdList, reset __vftable to the instMANAGER_HANDLER base slot.
    ~hcexHALO_CUSTOM_OBJ_LIST();

    // 0x823DCB88 — compiler-generated vector deleting destructor (`??_E` mangle): runs the
    ...
    // 0x823D97D0 — drop every pending create-data entry.
    void Clear();

    // 0x823DCAD8 — look up the pending create-data entry keyed by (sceneryIdx,typeId) and copy out
    ...
    // 0x823E5560 — decide whether a scene-instance creation should be intercepted by this handler:
    ...
    // 0x823D95B0 — true when the instance's class name is exactly "halo_no_model" (used by callers
