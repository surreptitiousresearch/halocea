...
struct game_engine
{
    ...
    uint8_t (__fastcall *initialize)(void);         /* 0x0C */
    ...
    void          (__fastcall *handle_client_message)(int, void *, int16_t);             /* 0x28 */
    void          (__fastcall *handle_server_message)(void *, int16_t);                  /* 0x2C */
    ...
    uint8_t (__fastcall *weapon_pickup)(int, int);    /* 0x40 */
    ...
    uint16_t *(__fastcall *get_score_string)(int, uint16_t *);        /* 0x54 */
    uint16_t *(__fastcall *get_score_header_string)(uint16_t *);       /* 0x58 */
    uint16_t *(__fastcall *get_team_score_string)(int, uint16_t *);    /* 0x5C */
    uint8_t (__fastcall *allow_pick_up)(int, int);    /* 0x60 */
    void          (__fastcall *player_damaged_player)(int, int, uint8_t);      /* 0x64 */
    void          (__fastcall *player_killed_player)(int, int, int, uint8_t);  /* 0x68 */
    uint8_t (__fastcall *rasterize_score)(int, int, int, uint16_t *, int); /* 0x6C */
    float         (__fastcall *starting_location_rating)(int, scenario_player *);      /* 0x70 */
    void          (__fastcall *prespawn_player_update)(int);  /* 0x74 */
    uint8_t (__fastcall *postspawn_player_update)(int); /* 0x78 */
    int           (__fastcall *game_engine_player_get_team_index)(int);               /* 0x7C */
    uint8_t (__fastcall *goal_matches_player)(int, int); /* 0x80 */
    uint8_t (__fastcall *game_engine_test_flag)(int);   /* 0x84 */
    uint8_t (__fastcall *game_engine_test_trait)(int, int); /* 0x88 */
    ...
    void          (__fastcall *player_changed_team)(int, uint8_t); /* 0x98 */
};                                              /* 156 bytes */
