/* message_weapon_start_reload_definition @ 0x84176278 (.data, 120 bytes) — the message-delta message definition for
 * `_message_weapon_start_reload` (id 40): a weapon has started reloading.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 4 body fields.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x836D9AD0 passes id 40.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000028 -> _message_weapon_start_reload
 ...
 * 0x841762F0, i.e. 120 bytes, and 0x28 + (4 + 1) * 16 == 120 — 16 being the DB member stride of
...
_message_definition message_weapon_start_reload_definition =
{
    _message_weapon_start_reload,            /* definition_type */
