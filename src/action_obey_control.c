...
void action_obey_control(int actor_index)
{
    ...
    if ( obey->complex_control.shoot_at_target )   /* forced move-to-point order active */
    {
        ...
        *(int *)&actor->orders.combat.target_point.x = *(int *)&obey->complex_control.shoot_target.x;       /* ordered destination -> move target */
        *(int *)&actor->orders.combat.target_point.y = *(int *)&obey->complex_control.shoot_target.y;
        *(int *)&actor->orders.combat.target_point.z = *(int *)&obey->complex_control.shoot_target.z;
    ...
    {
        __int16 stance = _idle_look_combat;
        unsigned __int8 explicit_facing = actor->state.flying;
        ...
        actor->orders.look.primary_direction.___u1.prop_index = *(int *)&obey->complex_control.destination_facing_point.x;   /* ordered look point */
        *(int *)&actor->orders.look.primary_direction.___u1.point.y = *(int *)&obey->complex_control.destination_facing_point.y;
        *(int *)&actor->orders.look.primary_direction.___u1.point.z = *(int *)&obey->complex_control.destination_facing_point.z;
        if ( !explicit_facing )
...
flush_order_effects:
    ...
    {
        unsigned __int16 animation_impulse = (unsigned __int16)obey->complex_control.action_animation_impulse;
        ...
        unsigned __int16 communication_type = (unsigned __int16)obey->complex_control.action_communication_type;
    ...
    if ( (obey->simple_control.simple_control_flags & (1u << _obey_simple_directmovement_bit)) != 0 )   /* directmovement (look/move) order */
    {
        ...
        *(int *)&actor->orders.move.override_movement_direction_vector.n[0] = *(int *)&obey->simple_control.___u5.directmovement.vector.i;   /* ordered look point */
        *(int *)&actor->orders.move.override_movement_direction_vector.n[1] = *(int *)&obey->simple_control.___u5.directmovement.vector.j;
        *(int *)&actor->orders.move.override_movement_direction_vector.n[2] = *(int *)&obey->simple_control.___u5.directmovement.vector.k;
    ...
    if ( (obey->simple_control.simple_control_flags & (1u << _obey_simple_jump_bit)) != 0 )   /* jump order */
    {
        ...
        if ( (obey->simple_control.simple_control_flags & (1u << _obey_simple_jump_jumped_bit)) != 0 )
        {
            ...
            __int16 aim_timer = obey->simple_control.___u5.jump.delay_ticks;
        ...
        if ( aim_set )
        {
            int aim_x = *(int *)&actor->input.facing_vector.n[0];
            int aim_y = *(int *)&actor->input.facing_vector.n[1];
            int aim_z = *(int *)&actor->input.facing_vector.n[2];
            actor->orders.move.override_movement_direction = 1;
            actor->orders.move.override_movement_facing = 0;
            *(int *)&actor->orders.move.override_movement_direction_vector.n[0] = aim_x;
            *(int *)&actor->orders.move.override_movement_direction_vector.n[1] = aim_y;
            *(int *)&actor->orders.move.override_movement_direction_vector.n[2] = aim_z;
        }
    }
}