/* message_local_player_update_definition @ 0x8417DE28 (.data, 104 bytes) — the message-delta message definition for
 * `_message_local_player_update` (id 34): the local player's own position update.
 * The message carries no per-message header (header_fields is the shared empty NO_HEADER set);
 * 3 body fields.
 * Encoder call site cross-checked in headers/message_delta_message_ids.h: 0x83756FD4 passes id 34.
 *
 * Per-field provenance, decoded big-endian straight from the image bytes:
 *   +0x00 definition_type                    = 0x00000022 -> _message_local_player_update
 ...
 *   +0x48   field_references[ 2]             = { 0x84183C08 -> &field_properties_point3d_definition, offset 4, baseline_offset 4, initialized 0 }
 ...
 * 0x8417DE90, i.e. 104 bytes, and 0x28 + (3 + 1) * 16 == 104 — 16 being the DB member stride of
...
_message_definition message_local_player_update_definition =
{
    _message_local_player_update,            /* definition_type */
    ...
    {                                        /* body_fields */
        ...
        {   /* field_references[count + terminator]: { properties, offset, baseline_offset, initialized } */
            ...
            { &field_properties_point3d_definition, 4, 4, 0 },                              /* [ 2] */
