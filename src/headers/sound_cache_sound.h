...
{
    int16_t                   identifier;       /* 0x00 — datum id (uncertain) */
    uint8_t           allocated;        /* 0x02 — record allocated/ready (set 1 on new) */
    uint8_t           loaded;           /* 0x03 — data present / preload complete */
    uint8_t           _unknown04;       /* 0x04 — uncertain */
    uint8_t           reference_count;  /* 0x05 — outstanding references */
    uint8_t           _unknown06;       /* 0x06 — cleared on load (uncertain) */
    uint8_t           _unknown07;       /* 0x07 — uncertain */
    int16_t                   _unused08;        /* 0x08 — cleared on new */
    int16_t                   _unknown0A;       /* 0x0A — uncertain */
