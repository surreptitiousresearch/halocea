...
void decals_reconnect_to_structure_bsp(void)
{
    ...
    while ( decal_index != -1 )
    {
        decal_datum *decal = &decals[(uint16_t)decal_index];
        ...
        if ( (uint16_t)decal_location.cluster_index != 0xFFFF )
        {
            int next_index = decal->next_decal_index;
            int previous_index = decal->prev_decal_index;

            if ( next_index != -1 )
                decals[(uint16_t)next_index].prev_decal_index = previous_index;

            if ( previous_index == -1 )
                decal_globals->first_disconnected_decal_index = next_index;
            else
                decals[(uint16_t)previous_index].next_decal_index = next_index;

            int16_t layer = decal->layer;
            int16_t cluster_index = decal_location.cluster_index;
            ...
            if ( old_head != -1 )
                decals[(uint16_t)old_head].prev_decal_index = decal_index;
