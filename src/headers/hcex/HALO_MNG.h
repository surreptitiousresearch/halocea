...
struct HALO_MNG : iaIACTOR {
    // 0x823D065C — default: base-constructs iaIACTOR; vtable installation is implicit (compiler-
    ...
    // 0x823D06AC — GetSuspendMask: this actor never contributes to the suspend mask. Protected
    // virtual override.
    virtual unsigned int GetSuspendMask();

    // 0x823D06C4 — OnChangeMode: chain to the base, then start/stop the Blam main-loop clock when
    // the "paused" mode bit (0x2) transitions (see main_start_time/main_stop_time). Protected
    // virtual override.
    virtual void OnChangeMode(unsigned int oldMode, unsigned int newMode);

    // 0x823E9F20 — ProcessFRAME: per-frame tick. On a dbg-triggered level-reload request, signals
    ...
    // 0x823DEA10 — ProcessMsg: on the ws-engine's per-frame "call frame" message (1016), clears the
    ...
    virtual msgRES ProcessMsg(int msg, void *pInfo, msgADDR *pSend);

    // 0x823DE98C — ProcessTERM_LEVEL: chain to the base, then tear down per-level HCEX state
    ...
    virtual void ProcessTERM_LEVEL();
};
